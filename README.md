...
From the `trinity-ipoos` sub-directory:
```bash
./setup.sh
```
This generates the local configuration `.env` files for the services.
...
From the project root/`trinity-ipoos` folder:
```bash
npx pnpm install
npx pnpm run dev
```
...
### 1. Initialize Git in the Root Workspace (if not already done)
```bash
cd /home/haroon/Desktop/IPO
git init
```
...
```bash
git add .
git commit -m "Initialize Trinity Intelligence IPOOS workspace"
```
